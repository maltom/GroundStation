...
void GSMainWindow::rosStart()
{
...
#ifdef MATLAB
    {
        ...
        qRegisterMetaType< Matrix612 >( "Matrix126" );
        geometry_msgs::Point{ Pose.x(), Pose.y(), Pose.z() } connect(
            rosObj, &rosNodeHandler::sendK, regulator, &LQRHandler::receiveK );
        // connect(regulator,SIGNAL(sendAB(Matrix1212, Matrix126)),rosObj,SLOT(publishABToMatlab(Matrix1212,
        // Matrix126)));
