...
void ROV::init_geometry()
{
    ...
    // Mass
    m = 19.0;
    // Location of CoG in relation to Center of Origin of axes. It is set to 0.02 in z as it is located at Center of
    // Buoy.
    rg << 0, 0, 0.02;
...
void ROV::init_thrust()
{
    ...
    t1 << cos( alpha01 ), sin( alpha01 ), 0.0, sin( alpha01 ) * ( -0.1235 ), cos( alpha01 ) * 0.1235,
        ( sin( alpha01 ) * ( 0.038 ) ) - ( cos( alpha01 ) * ( -0.165 ) );
    t2 << cos( alpha02 ), sin( alpha02 ), 0.0, sin( alpha02 ) * ( -0.1235 ), cos( alpha02 ) * 0.1235,
        ( sin( alpha02 ) * ( 0.038 ) ) - ( cos( alpha02 ) * ( 0.165 ) );
...
Matrix< double, 12, 12 > ROV::A_state_matrix( VectorXd cur_state )
{
    ...
    // State Space matrix
    A << MatrixXd::Zero( 6, 6 ), MatrixXd::Identity( 6, 6 ), MatrixXd::Zero( 6, 6 ), -damping_coeffs;
...
VectorXd ROV::getRestoringForces( VectorXd currentState )
{
    ...
    RestoringForces << ( Wght - Buoy ) * sin( th ), -( Wght - Buoy ) * cos( th ) * sin( ph ),
        -( Wght - Buoy ) * cos( th ) * cos( ph ), rg( 2 ) * Wght * cos( th ) * sin( ph ), rg( 2 ) * Wght * sin( th ),
        0.0;
